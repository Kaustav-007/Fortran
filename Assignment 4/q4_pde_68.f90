program laplace_sor
    ...
    !------------------------------------------------------------------------
    integer, parameter :: N = 68                  ! Changed from 34 to 68
    ...
    B = -40.0d0  ! dT/dx at x = 68 (changed from 34)
    C =  20.0d0  ! dT/dy at y = 1
    D = -10.0d0  ! dT/dy at y = 68 (changed from 34)
    ...
    do while (max_diff > CONV_LIMIT .and. iterations < MAX_ITER)
       ...
       if (mod(iterations, 5000) == 0) then
          write(*,'(A,I0,A,ES12.4)') "Iteration ", iterations, &
                ", max_diff = ", max_diff
          write(*,'(A,ES12.4)') "T(20,20) = ", T(20,20)    ! Changed from (10,10) to (20,20)
       end if
    ...
    ! Print final temperature at (20,20)
    write(*,'(A,ES12.4)') "Temperature at (20,20): ", T(20,20)    ! Changed from (10,10) to (20,20)
 
    !------------------------------------------------------------------------
    ! Save the final temperature profile to a file
    !------------------------------------------------------------------------
    open(unit=10, file='temperature_profile.txt', status='unknown', action='write')
    write(10,*) "Temperature Profile (68x68 grid):"    ! Updated from 34x34 to 68x68
    do i = 1, N
       do j = 1, N
