...
def save_A_video():
    pattern, iterations = load_data('initialize.txt')
    
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.set_title('Turing Pattern - Species A')
    ...
    plt.colorbar(im, label='Concentration A')
    
    def update(frame):
        im.set_array(pattern[frame])
        ax.set_title(f'Species A (Iteration {iterations[frame]})')
        return im,
    
    print("Rendering Species A video...")
    with writer.saving(fig, "turing_pattern_A.mp4", dpi=100):
        for frame in range(len(iterations)):
            update(frame)
            writer.grab_frame()
    print("Video saved as 'turing_pattern_A.mp4'")
